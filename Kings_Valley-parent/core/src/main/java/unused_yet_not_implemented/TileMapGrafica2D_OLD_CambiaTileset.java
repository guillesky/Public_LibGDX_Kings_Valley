...
public class TileMapGrafica2D_OLD_CambiaTileset implements IMyApplicationListener
{
    ...
    protected Array<AnimatedTrapKV2D> animatedTraps = new Array<AnimatedTrapKV2D>();
    ...
    {
	/*
	 * this.spriteBatch.begin(); ArrayIterator<AnimatedTrapKV2D> it3 =
	 * this.animatedTraps.iterator(); while (it3.hasNext()) { AnimatedTrapKV2D
	 * animatedTrapKV2 = it3.next(); animatedTrapKV2.updateElement(null);
	 * 
	 * } if (Game.getInstance().getState() == Game.ST_GAME_PLAYING) {
	 * Iterator<AbstractAnimatedDoor2D> itDoors = this.animatedDoor2D.iterator();
	 * while (itDoors.hasNext()) { AbstractAnimatedDoor2D animatedDoor =
	 * itDoors.next(); animatedDoor.updateElement(null);
	 * animatedDoor.draw(spriteBatch); } } ArrayIterator<MySpriteKV> it =
	 * this.arrayOfMySpritesKV.iterator(); while (it.hasNext()) { MySpriteKV
	 * mySpriteKV = it.next(); mySpriteKV.updateElement(null);
	 * mySpriteKV.draw(spriteBatch); }
	 * 
	 * ArrayIterator<AnimatedEntity2D> it2 = this.animatedEntities.iterator(); while
	 * (it2.hasNext()) { AnimatedEntity2D animatedEntity2D = it2.next();
	 ...
	 * this.playerAnimated2D.render(spriteBatch); } else if
	 * (Game.getInstance().getState() != Game.ST_ENDING) {
	 * this.animatedEnteringDoor2D.updateElement(null);
	 * this.animatedEnteringDoor2D.drawBack(spriteBatch);
	 * this.playerAnimated2D.updateElement(Game.getInstance().getDelta()); if
	 * ((Game.getInstance().getState() == Game.ST_GAME_ENTERING &&
	 ...
	 * this.animatedEnteringDoor2D.drawFront(spriteBatch); } else {
		...
		|| Game.getInstance().getRenderMode() == Game.ST_ENDING)
	{
	    ...
	    if (Game.getInstance().getRenderMode() == Game.ST_GAME_ENTERING)
	    {
		textToShow =Game.getInstance().getTextToEnterLevel();
		/*
		 * if (Game.getInstance().isGoingBack()) textToShow =
		 * Messages.GOING_BACK.getValue(); else textToShow =
		 * Messages.ENTERING.getValue(); textToShow += currentPyramid;
		 */
	    } else
	    {
