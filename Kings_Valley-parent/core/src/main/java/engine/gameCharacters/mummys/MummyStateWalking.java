...
public class MummyStateWalking extends MummyState
{
	...
	{
		if (this.mummy.isFeetColision(this.platformAnalysisResult.getEndPlatformLeft().getRectangle())
				&& this.mummy.getDirection().x == MummyState.LEFT)
			this.analizeEndPlatform(this.platformAnalysisResult.getEndPlatformLeft());
		else if (this.mummy.isFeetColision(this.platformAnalysisResult.getEndPlatformRight().getRectangle())
				&& this.mummy.getDirection().x == MummyState.RIGHT)
			this.analizeEndPlatform(this.platformAnalysisResult.getEndPlatformRight());

	}

	private void analizeEndPlatform(EndPlatform endPlatformLeft)
	{

		if (endPlatformLeft.getType() == EndPlatform.END_BLOCK) // SI hay un bloqueo
		...
		else
		{
			...
			if (endPlatformLeft.getType() == EndPlatform.END_STEP) // Si es un bloque escalon, es decir se puede saltar)
			{
