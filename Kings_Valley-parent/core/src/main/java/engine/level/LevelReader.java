...
public class LevelReader
{
	...
	this.unpickableCells = new HashSet<Cell>();
	...
	for (int i = 0; i < layerObject.getObjects().getCount(); i++)
	{
	    ...
	    { // Calculo la altura y ancho de las escaleras
		width = GameRules.getInstance().getStairWidth();
		height = GameRules.getInstance().getStairHeight();
	    } else if (type == Constants.IT_GIRATORY)
	    {// Calculo la altura de la puerta giratoria y agrego las celdas superiores a las
	     // NO picables
		...
		this.unpickableCells.add(this.getCell(map, fx + GameRules.getInstance().getGiratoryWidth(),
			fy + GameRules.getInstance().getLevelTileHeightUnits() * contador));
