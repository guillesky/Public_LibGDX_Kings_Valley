...
public class Facade implements ApplicationListener
{
    ...
    /**
     * Retorna el fileHandle del archivo de Screenshot dentro de la carpeta APPDATA
     * del sistema
     * 
     * @return El archivo de configuracion dentro de la carpeta APPDATA del sistema
     */
    private FileHandle getScreenshotFile()
    {
	String appData = System.getenv("APPDATA");
	String lastFolder = "KingsValley" + File.separator + "screenshots";
	FileHandle screenshotDir;

	if (appData != null)
	{
	    // Windows
	    screenshotDir = Gdx.files.absolute(appData + File.separator + lastFolder);
	} else
	{
	    // Linux y otros sistemas
	    String home = System.getProperty("user.home");
	    screenshotDir = Gdx.files.absolute(
		    home + File.separator + ".local" + File.separator + "share" + File.separator + lastFolder);
	}
	...
	String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy_MM_dd_HHmmss_SSS"));
