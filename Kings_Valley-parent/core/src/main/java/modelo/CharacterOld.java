...
public abstract class CharacterOld extends LevelItem
{
    ...
    protected int state = CharacterOld.st_init;
    ...
    public CharacterOld(int type, float x, float y, int p0, int p1, float width, float height, TiledMapTileLayer layer,
	...
	if (!this.isFloorDown())
	{
	    this.state = CharacterOld.st_falling;
	...
	else
	{
	    ...
	    this.state = CharacterOld.st_walk;
	    this.motionVector.y = 0;
	    if (b)
	    {
		this.motionVector.y = this.speedJump;
		this.state = CharacterOld.st_jump_top;
	    }
	}
    ...
    {
	if (this.state != CharacterOld.st_walk)
	{
