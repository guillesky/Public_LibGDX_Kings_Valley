 ...
 * crear plantillas de imagenes (usada para la creacion de recursos)
 * 
...
public class PNGMerger extends ApplicationAdapter
{
	...
	this.plantillaDeUnaImagen(pngFiles.get(0), 7, 8, 110, 114);
	Gdx.app.exit();
    }

    private void plantillaDeUnaImagen(FileHandle pngFile, int cant_h, int cant_v, int offset_h, int offset_v)
    {
	Pixmap current = new Pixmap(pngFile);
	int singleWidth = current.getWidth();
	int singleHeight = current.getHeight();

	Pixmap result;
	int xMargin = (offset_h - singleWidth) / 2;
	int yMargin = (offset_v - singleHeight) / 2;

	result = new Pixmap(offset_h * cant_h, offset_v * cant_v, Pixmap.Format.RGBA8888);
	for (int i = 0; i < cant_v; i++)
	    for (int j = 0; j < cant_h; j++)

		result.drawPixmap(current,xMargin+ j * offset_h, yMargin+i * offset_v);

	current.dispose();

	// Guardar la imagen final
	FileHandle output = Gdx.files.local("output/plantilla.png");
