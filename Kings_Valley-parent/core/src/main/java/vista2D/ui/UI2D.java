...
public class UI2D implements IView, ApplicationListener
{
	...
	private Texture currentPiramydTexture;
	private Texture completedPiramydTexture;
	...
	private void getResources()
	{
		...
		this.mapTexture = manager.get(this.uiConfig.getMapFile(), Texture.class);
		this.currentPiramydTexture = manager.get(this.uiConfig.getPiramydActualFile(), Texture.class);
		this.completedPiramydTexture = manager.get(this.uiConfig.getPiramydCompletedFile(), Texture.class);
	...
	public void resize(int width, int height)
	{
		...
		if (this.mapSprite != null)
			this.calulateMapSprite(width, height);
	...
	private void calulateMapSprite(float width, float height)
	{
		...
		this.mapSprite.setPosition((width - this.mapSprite.getWidth()) / 2f,
				(height - this.mapSprite.getHeight()) / 2f);
	...
	public void doUiInGame()
	{
		...
		this.stage.getRoot().removeActor(this.tableMainActual);
		this.stage.getRoot().removeActor(this.mapImage);
...
//	int currentLevel = Game.getInstance().getCurrentLevel().getId();
	...
	{/*
		 * this.createMap(); this.calulateMap(this.mapImage, getMasterVolume(),
		 * getDificultLevel()); this.createMapTable();
		 */
	}
