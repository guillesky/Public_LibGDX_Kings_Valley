 ...
 */
public  class AbstractUIMap 
{
	protected Texture mapTexture;
	protected Texture currentPyramidTexture;
	protected Texture completedPyramidTexture;
	protected Sprite mapSprite;
	protected SpriteBatch batch;
	protected OrthographicCamera cameraMap;
	protected HashMap<Integer, CellInMap> pyramidsInMap = new HashMap<Integer, CellInMap>();

	protected SpriteWithId currentPyramidSprite;

	protected ArrayList<SpriteWithId> completedPyramidSprites = new ArrayList<SpriteWithId>();
	protected int cantDivision;
	 ...
	 */
	public AbstractUIMap(Texture mapTexture, Texture currentPyramidTexture, Texture completedPyramidTexture,
			int cantDivision)
	{
		super();
		...
		this.cantDivision = cantDivision;
		this.generatePyramidsCellsInMaps();
		this.generateCompletedPyramids();
	...
	private void calulateMapSprites(float width, float height)
	{
		...
		if (imageRatio > stageRatio)
		{
			...
			cellSize = width / this.cantDivision;
		} else
		{
			this.mapSprite.setSize(height * imageRatio, height);
			cellSize = height / this.cantDivision;
		}
	...
	public void render()
	{
		...
		System.out.println(this.currentPyramidSprite);
	...
	public void generateCompletedPyramids()
	{
		...
		if (Game.getInstance().getCurrentLevel() != null)
			id = Game.getInstance().getCurrentLevel().getId();
		this.currentPyramidSprite = new SpriteWithId(currentPyramidTexture, id);
