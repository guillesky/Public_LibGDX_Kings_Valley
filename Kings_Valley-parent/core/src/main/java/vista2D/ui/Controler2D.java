...
public class Controler2D extends AbstractControler implements KVEventListener
{
	...
	this.ui.createMap(this.view.getGameType());
	...
	case KVEventListener.FINISH_GREAT_TEMPLE:
	{
	    int greatTempleFinished = (int) param;
	    doFinishGreatTemple(greatTempleFinished);
	    break;
	}
    ...
    private void doFinishGreatTemple(int greatTempleFinished)
    {
	Facade.getInstance().finishGreatTemple(greatTempleFinished);
