...
//		renderer.renderTileLayer((TiledMapTileLayer) renderer.getMap().getLayers().get("back"));
	...
	if (aux_X >= (cameraBack.viewportWidth / 2)
		&& aux_X + (cameraBack.viewportWidth / 2) <= pyramid.getMapWidthInPixels())
	...
	else
	{
		...
		if (aux_X < pyramid.getMapWidthInPixels() / 2)
		    posCameraX = cameraBack.viewportWidth / 2;
		else
		    posCameraX = pyramid.getMapWidthInPixels() - cameraBack.viewportWidth / 2;
	    }
	}
