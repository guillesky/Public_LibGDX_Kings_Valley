...
public class TileMapGrafica2D implements IMyApplicationListener
{
	...
	cameraFront = new OrthographicCamera(pyramid.getMapHeightInUnits() * 4 / 3, pyramid.getMapHeightInUnits());

	cameraFront.position.x = pyramid.getMapWidthInUnits() * .5f;
    ...
    {
	Pyramid pyramid = Game.getInstance().getCurrentLevel().getPyramid();
	float alto = 24 * GameRules.getInstance().getLevelTileHeightUnits();
	...
	cameraFront.update();
	this.calculateCamera(this.cameraFront, 0);
	this.calculateCamera(this.cameraBack, this.factor);
	...
	if (GameRules.getInstance().isDebugMode())
	{
	    ...
	    shapeRenderer.setProjectionMatrix(cameraFront.combined);
	    while (it.hasNext())
		it.next().render();
	}
	this.drawUI();
    ...
    {
	spriteBatch.setProjectionMatrix(cameraFront.combined);
	this.spriteBatch.begin();
    ...
    /**
     * Llamado internamente por render(). Calcula la camara pasada por parametro con
     * el factor de paralaz pasado por parametro
     */
    private void calculateCamera(OrthographicCamera camera, float factor)
    {
	Pyramid pyramid = Game.getInstance().getCurrentLevel().getPyramid();
	...
	if (pyramid.getMapHeightInTiles() > 22 && playerY >= (camera.viewportHeight / 2)
    ...
    /**
     * Llamado internamente por resize(). Calcula la camara de la piramide pasasda
     * por parametro considerando el factor de paralax pasado por parametro. Realiza
     * mas calculos que calculateCamera, ya que considera los cambios ocurridos
	...
	float medio = mapSizeInUnits / 2;
	float dif = (medio - playerCoord) * factor;
	float aux = playerCoord + dif;

	if (playerCoord >= (viewPortSize / 2) && playerCoord + (viewPortSize / 2) <= mapSizeInUnits)
	    posCamera = aux;

	else
	{
	    if (viewPortSize >= mapSizeInUnits)
		posCamera = mapSizeInUnits * .5f;
	    else
	    {
		if (playerCoord < mapSizeInUnits / 2)
		{

		    playerCoord = viewPortSize / 2;

		} else
		{
		    playerCoord = mapSizeInUnits - viewPortSize / 2;

		}
		medio = mapSizeInUnits / 2;
		dif = (medio - playerCoord) * factor;
		posCamera = playerCoord + dif;
	    }
	}
	return posCamera;
